...
public interface SaveRebalancingRepository extends JpaRepository<SaveRebalancing, Integer> {
    ...
    // 특정 사용자의 특정 계좌 기록 조회
    @Query("SELECT s FROM SaveRebalancing s WHERE s.user_id = :user_id AND s.account = :account ORDER BY s.record_date DESC")
    ...
    // 특정 사용자의 모든 기록 조회
    @Query("SELECT s FROM SaveRebalancing s WHERE s.user_id = :user_id ORDER BY s.record_date DESC")
    List<SaveRebalancing> findByUserIdOrderByRecordDateDesc(@Param("user_id") String user_id);

    // 날짜 범위 조회
    @Query("SELECT s FROM SaveRebalancing s WHERE s.user_id = :user_id AND s.account = :account AND s.record_date BETWEEN :startDate AND :endDate ORDER BY s.record_date DESC")
